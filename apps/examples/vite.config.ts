...
const env = getEnv()
const DEFAULT_AI_BASE_URL = 'https://apihub.agnes-ai.com'
const DEFAULT_TEXT_MODEL = 'agnes-2.0-flash'
const DEFAULT_IMAGE_MODEL = 'agnes-image-2.0-flash'
const TEXT_MODEL_IDS = new Set(['agnes-2.0-flash', 'agnes-1.5-flash'])
const IMAGE_MODEL_IDS = new Set(['agnes-image-2.0-flash', 'gpt-image-2', 'nanobanana', 'nanobanana-pro', 'nanobanana-2'])

const AGNES_VIDEO_BASE_URL = 'https://apihub.agnes-ai.com'
const DEFAULT_VIDEO_MODEL = 'agnes-video-v2.0'
const VIDEO_MODEL_IDS = new Set(['agnes-video-v2.0'])
...
function aiStudioApiPlugin(): Plugin {
	return {
		...
		configureServer(server) {
			const PREVIEW_REDIRECT = { redirect: '/' }
			// Reuse the 2000+ lines of middleware from preview mode too
			...
			server.middlewares.use('/api/analyze-image-prompt', async (req, res) => {
				...
				try {
					...
					// Use Vision API (NewAPI + Gemini) when configured, fall back to main API
					const visionAuth = getVisionAuthorizationHeader()
					const useVision = Boolean(visionAuth)
					const authHeader = useVision ? visionAuth : getAiAuthorizationHeader()
					const endpoint = useVision
						? getVisionEndpoint('chat/completions')
						: getAiEndpoint('chat/completions')
					const visionModel = process.env.VISION_MODEL?.trim() || DEFAULT_VISION_MODEL
					const model = useVision ? visionModel : (getString(body.model) || DEFAULT_TEXT_MODEL)
					...
					if (!useVision && !isTextModelAllowed(model)) {
						sendJson(res, 400, {
			...
			server.middlewares.use('/api/generate-video', async (req, res) => {
				...
				try {
					const authHeader = getAiAuthorizationHeader()
					if (!authHeader) {
						sendJson(res, 503, { error: 'IMAGE_API_KEY 未配置，请先在接口配置面板填写。' })
						return
					}
					...
					const prompt = getString(body.prompt).trim()
					const ratio = getString(body.ratio) || '16:9'
					...
					}[ratio] || { width: 1152, height: 648 }
					// Build request body for Agnes-Video-V2.0
					interface VideoImageInput { url: string; role?: string }
					const videoImages = (images as VideoImageInput[])
						.map((img: VideoImageInput) => img.url)
						.filter(Boolean)
					...
					if (videoImages.length === 1) {
						videoBody.image = videoImages[0]
					} else if (videoImages.length > 1) {
						// Agnes API requires `mode=keyframes` when 2..N images are sent
						videoBody.mode = 'keyframes'
						videoBody.image = videoImages
					}

					const response = await fetchWithTimeout(
						`${AGNES_VIDEO_BASE_URL}/v1/videos`,
						{
							method: 'POST',
							headers: {
								Authorization: authHeader,
					...
					if (!response.ok) {
						const message = getAiError(data) || `视频任务创建失败（HTTP ${response.status}）`
						console.error('[ai-studio-api] Video task creation failed:', message)
						sendJson(res, response.status, { error: message })
						return
					}
					const taskId = getString(data?.id || data?.task_id)
					const videoId = getString(data?.video_id) || taskId
					if (!taskId) {
						sendJson(res, 502, { error: 'Agnes API 没有返回任务 ID。' })
						return
					}
					sendJson(res, 200, { taskId, videoId })
				} catch (err) {
					const message = getErrorMessage(err)
			...
			server.middlewares.use('/api/video-task', async (req, res) => {
				...
				if (!taskId) {
					sendJson(res, 400, { error: '请提供视频 ID。' })
					return
				}
				const authHeader = getAiAuthorizationHeader()
				if (!authHeader) {
					sendJson(res, 503, { error: 'AI API Key 未配置，请在接口配置面板填写。' })
					return
				}
				...
				if (req.method === 'DELETE') {
					try {
						...
						if (!response.ok) {
							const message =
								getAiError(parseJsonSafely(responseText)) ||
				...
				try {
					...
					// Use video_id query (recommended) — fall back to task_id path
					const isVideoId = taskId.startsWith('video_')
					const queryUrl = isVideoId
						? `https://apihub.agnes-ai.com/agnesapi?video_id=${encodeURIComponent(taskId)}`
					...
					const data = parseJsonSafely(responseText)
					if (!response.ok) {
						const message = getAiError(data) || `查询任务失败（HTTP ${response.status}）`
						sendJson(res, response.status, { error: message })
						return
					}
					...
					function pickRemoteVideoUrl(data: any): string {
    ...
    for (const c of candidates) {
        const v = getString(c).trim()
        if (v) return v
    }
    return ''
}
const remoteVideoUrl = pickRemoteVideoUrl(data)
					if (!remoteVideoUrl) {
...
// ---- Vision API (for reverse prompt / image understanding) ----
...
function getVisionApiKey() {
	return process.env.VISION_API_KEY || process.env.NEWAPI_API_KEY
}
function getVisionAuthorizationHeader() {
	const apiKey = getVisionApiKey()?.trim()
	if (!apiKey) return ''
	return apiKey.toLowerCase().startsWith('bearer ') ? apiKey : `Bearer ${apiKey}`
}
function getVisionBaseUrl() {
	return normalizeAiBaseUrl(process.env.VISION_API_BASE_URL || process.env.NEWAPI_BASE_URL) || 'https://newapi.dzkjm.cn/v1'
}
function getVisionEndpoint(pathname: string) {
	const baseUrl = getVisionBaseUrl().replace(/\/+$/, '')
	const versionedBaseUrl = baseUrl.endsWith('/v1') ? baseUrl : `${baseUrl}/v1`
	return `${versionedBaseUrl}/${pathname.replace(/^\/?(v1\/)?/, '')}`
}
function isVisionModelAllowed(model: string) {
	return VISION_MODEL_IDS.has(model)
}
// ---- End Vision API ----
...
async function downloadVideoToCache(remoteUrl: string, localPath: string) {
	// Agnes CDN returns invalid "content-encoding: utf-8" headers which causes
	// Node.js fetch (undici) to fail with "error decoding response body".
	// Use native http/https modules which do NOT auto-decompress based on
	// content-encoding, avoiding the stream decode error entirely.
	...
	await new Promise<void>((resolve, reject) => {
		...
		const request = transport.get(remoteUrl, { timeout: timeoutMs }, (res) => {
			// Follow redirects (3xx)
			if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
				request.destroy()
				downloadVideoToCache(res.headers.location, localPath).then(resolve, reject)
				return
			}
		...
		request.on('timeout', () => {
			request.destroy()
			try { unlinkSync(tmpPath) } catch { /* ignore */ }
			reject(new Error(`视频下载超时（${Math.round(timeoutMs / 1000)}秒）`))
		})
...
function filterGatewayModels(modelIds: string[]) {
	const textModels = sortModelsFirst(
		modelIds.filter(isTextModelAllowed).map(toModelOption),
		DEFAULT_TEXT_MODEL
	)
	const imageModels = sortModelsFirst(
		modelIds.filter(isImageModelAllowed).map(toModelOption),
		DEFAULT_IMAGE_MODEL
	)
	return {
...
function sortModelsFirst(options: { id: string; label: string }[], preferredId: string) {
	const preferred = options.filter((m) => m.id === preferredId)
	const rest = options.filter((m) => m.id !== preferredId)
...
function isNanoBananaModel(model: string) {
	return model.startsWith('nanobanana')
}
