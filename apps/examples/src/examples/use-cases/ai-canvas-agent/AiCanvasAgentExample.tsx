...
const VIDEO_POLL_NETWORK_TIMEOUT_MS = 120_000 // 连续网络失败 2 分钟后自动取消
...
export default function AiCanvasAgentExample() {
	...
	async function pollVideoTask(nodeId: string, taskId: string) {
		...
		let networkFailSince: number | null = null
		try {
			while (true) {
				...
				try {
					...
					if (networkFailSince === null) {
						networkFailSince = Date.now()
					} else if (Date.now() - networkFailSince >= VIDEO_POLL_NETWORK_TIMEOUT_MS) {
						cancelledVideoTasksRef.current.add(taskId)
						updateVideoNode(nodeId, {
							status: 'failed',
							errorMessage: '网络连接超时（超过 2 分钟），任务已自动取消',
						})
						// 尝试取消远端任务（不阻塞）
						fetch(`/api/video-task?id=${encodeURIComponent(taskId)}`, { method: 'DELETE' }).catch(() => {})
						return
					}
